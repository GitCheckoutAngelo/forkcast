...
Convert imperial weight units to metric: oz→g (1 oz=28g), lbs→g (1 lb=454g). Also convert fl oz→ml (1 fl oz=30ml) and pints→ml (1 pint=480ml). Keep cups, tsp, and tbsp as-is — do not convert them to ml. After converting, apply scale-up rounding: if result ≥500g express in kg rounded to 1 decimal (e.g. 2 lbs→0.9kg, 3 lbs→1.4kg); if result ≥1000ml express in L rounded to 1 decimal (e.g. 5 cups→1.2L). Already-metric units (g, kg, ml, l) stay unchanged. Non-volume/weight units (piece, clove, bunch, slice, sprig, pinch, can, head) stay unchanged.`

...
- **Parse stage (`/api/recipes/parse-ingredients`):** Claude extracts quantity and unit *as written in the source text* (e.g. `1, "lb"`) — no conversion, no arithmetic. `normalizeQty` from `src/lib/recipes/metric.ts` then converts and rounds deterministically in TypeScript before the response is returned. Do not move this math back into the prompt.
- **Extract stage (`/api/recipes/extract`):** Both the JSON-LD fast path and the Claude fallback return the raw candidate without normalizing ingredients. The UI always auto-triggers re-parse on first open (`autoReparseIngredients: true`), which runs the parse stage above and is where normalization happens.
- **Grocery list stage (`/api/grocery-lists/generate`):** `normalizeQty` converts and rounds deterministically in TypeScript *before* contributions reach Claude, and `cleanQuantityText` re-rounds Claude's output strings after. The Claude prompt also includes conversion instructions — this is intentional and not redundant: ingredients with a null `quantity` fall back to their `raw_text` string, which may contain imperial units from manual user entry that bypassed re-parse. The prompt covers oz/ounce/ounces, lb/lbs/pound/pounds, fl oz/fluid ounce, pint/pints/pt, and quart/quarts/qt — keep these in sync with `IMPERIAL_CONVERSIONS` in `src/lib/recipes/metric.ts`.
